...
2. Sometimes when re-running the `python fill_database.py` command, it will cause error. The log from the error is available here. I think it's probably something that keeps the `*.db` open, and thus, can't write new information on it? It's definitely a sqlite issue, not my application's issue.
3. I'm not checking if there are any values what so ever when creating hospitals or conditions. This means that you can create something completely blank. But why would you do that to me?
